<template>
    <div>
        ...
        <h1>Cadastrar categorias de despesas</h1>
        ...
        <v-form ref="form" v-model="valid">
            <v-row>
                ...
                ></Input>
                <InputNumber
                    label="Porcentagem destinada para a categoria (controle)"
                    :stepInt="true"
                    :min="1"
                    :max="100"
                    :value="categoria.porcent"
                    @update="categoria.porcent = $event"
                ></InputNumber>
            </v-row>
...
export default {
    ...
    mounted() {
        if (this.$route.path == "/editar-categoria") {
            if (this.$store.getters["Categoriasdespesa/getEditCategoria"] == null) this.$router.push({ path: "/categorias" });

            this.edit = true;
            this.categoria = {...this.$store.getters["Categoriasdespesa/getEditCategoria"]};
    ...
    methods: {
        cadastrar() {
            if (this.$refs.form.validate()) {
                ...
                this.$store.dispatch("Categoriasdespesa/createCategoria", data).then(
                    response => {
                        this.$router.go(-1);
                    },
                    error => {
